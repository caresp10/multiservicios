        ...
        <tr class="${!servicio.activo ? 'table-secondary' : ''}">
            ...
            <td>
                ...
                <button class="btn btn-sm btn-outline-${servicio.activo ? 'warning' : 'success'}"
                        onclick="toggleEstadoServicio(${servicio.idServicio}, ${servicio.activo})"
                        title="${servicio.activo ? 'Desactivar' : 'Activar'}">
                    <i class="fas fa-${servicio.activo ? 'toggle-on' : 'toggle-off'}"></i>
                </button>
            </td>
        </tr>
    `).join('');
...
async function cargarCategorias() {
    try {
        ...
        if (response.success && response.data) {
            ...
            // Agregar evento para generar código automáticamente al cambiar categoría
            selectForm.addEventListener('change', async function() {
                const idCategoria = this.value;
                if (idCategoria && !document.getElementById('idServicio').value) {
                    // Solo generar código si es un nuevo servicio (no edición)
...
function openModalServicio() {
    ...
    // Limpiar y habilitar campo código para nuevo servicio
    document.getElementById('codigo').value = '';
    document.getElementById('codigo').readOnly = true;
    document.getElementById('codigo').placeholder = 'Se genera automáticamente';
...
async function toggleEstadoServicio(id, estadoActual) {
    ...
    const accionCapital = estadoActual ? 'Desactivar' : 'Activar';

    const result = await Swal.fire({
        title: '¿Está seguro?',
        text: `El servicio será ${accion}do`,
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: estadoActual ? '#f0ad4e' : '#28a745',
    ...
    try {
        ...
        if (response.success) {
            await Swal.fire(
                `${accionCapital}do`,
                `El servicio ha sido ${accion}do correctamente`,
                'success'
            );
            cargarServicios();
        } else {
            await Swal.fire('Error', response.message || `Error al ${accion} el servicio`, 'error');
        }
    } catch (error) {
        console.error('Error:', error);
        await Swal.fire('Error', `Error al ${accion} el servicio`, 'error');
    }
}
...
async function generarCodigoAutomatico(idCategoria) {
    try {
        ...
        if (result.success && result.data) {
            document.getElementById('codigo').value = result.data;
        } else {
            console.error('Error generando código:', result.message);
            // Si falla, permitir ingreso manual
            document.getElementById('codigo').readOnly = false;
            document.getElementById('codigo').placeholder = 'Ingrese código manualmente';
        }
    } catch (error) {
        console.error('Error:', error);
        // Si falla, permitir ingreso manual
